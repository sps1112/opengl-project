...
void main()
{
    ...
    {
        emmision = vec3(texture(material.texture_emmision1,TexCoord));
    }
...
vec3 GetAmbient(vec3 ambient)
{
    vec3 final = ambient * vec3(texture(material.texture_diffuse1,TexCoord));
    return final;
}
...
vec3 GetDiffuse(vec3 normal, vec3 lightDir, vec3 diffuse)
{
    ...
    vec3 final = diffuse * (diff * vec3(texture(material.texture_diffuse1,TexCoord)));
    return final;
}
...
vec3 GetSpecular(vec3 normal, vec3 lightDir, vec3 viewDir, vec3 specular)
{
    ...
    vec3 final = specular * (spec * (vec3(texture(material.texture_specular1,TexCoord))));
    return final;
}